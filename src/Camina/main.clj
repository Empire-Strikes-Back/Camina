      ...
      (when-let [value (<! ops|)]
        (condp = (:op value)
          ...
          (let [{:keys [frequency role]} value
                ...
                games-topic (Camina.corn/encode-base64url-u "raisins")
                game-topic (Camina.corn/encode-base64url-u frequency)
                _ (Camina.corn/subscribe-process
                   {:sub| sub|
                    :cancel| cancel-sub|
                    :frequency frequency
...
(defn -main
  ...
  (let [data-dir-path (or
                       ...
                       (.getCanonicalPath ^File (Wichita.java.io/file (System/getProperty "user.home") ".Camina")))
        state-file-path (.getCanonicalPath ^File (Wichita.java.io/file data-dir-path "Camina.edn"))]
